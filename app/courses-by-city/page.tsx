...
const TOP_CITIES = ALL_CITIES.filter(c => c.tier === 1).slice(0, 20)
...
export default function CoursesByCityPage() {
  ...
  return (
    <main>
      ...
      {/* Complete Course-City Directory (SEO Indexation path) */}
      <section className="py-16 bg-[#0a0a12] relative border-t border-white/[0.04]">
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-black text-white mb-4">Complete Location <span className="gradient-text">Directory</span></h2>
          <p className="text-sm text-[#94a3b8] mb-8">Select a course and city to view localized placement data and curriculum information.</p>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {COURSES.map(course => (
              <div key={course.slug} className="glass rounded-xl p-5 border border-white/[0.06] hover:border-[#6366f1]/20 transition-all flex flex-col justify-between">
                <div>
                  <h3 className="text-sm font-bold text-white mb-3 flex items-center gap-2">
                    <span className="text-lg">{course.icon}</span> {course.title}
                  </h3>
                  <div className="flex flex-wrap gap-1.5 max-h-[160px] overflow-y-auto pr-1 scrollbar-thin">
                    {ALL_CITIES.filter(city => INDEXABLE_CITY_SLUGS.has(city.slug))
                      .sort((a, b) => a.name.localeCompare(b.name))
                      .map(city => (
                        <Link
                          key={city.slug}
                          href={`/courses/${course.slug}/${city.slug}/`}
                          className="text-[10px] bg-white/[0.02] hover:bg-white/[0.06] px-2.5 py-1 rounded-full border border-white/[0.04] text-[#94a3b8] hover:text-white transition-all"
                        >
                          {city.name}
                        </Link>
                      ))}
